...
#'
#' @return The original \code{x} with tow additional columns named \code{"aicc_loess_value"} and \code{"aicc_span"}
...
loess_aicc_optimized <- function(x
  ...
  for (g in group_subset) {
    ...
    x[eval(parse(text = groups)) == g & eval(parse(text = independent_var)) %in% independent_var_range, aicc_loess_value:= predict(loess_model_optimum)]
